...
export function LoginPage() {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setError(''); setBusy(true); try { const nextProfile = await signIn(email, password); navigate(nextProfile ? ROUTES.home : ROUTES.profileSetup, { replace: true }); } catch (err) { setError(err instanceof Error ? err.message : 'Could not sign in.'); } finally { setBusy(false); } }
